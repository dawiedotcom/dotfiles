...
"
" Take a look at 
" [1]: http://vim.sourceforge.net/scripts/script.php?script_id=116
" [2]: https://arun.wordpress.com/2009/04/10/c-and-vim/
" [3]: http://www.schmid.dk/wiki/index.php/Unity_and_Vim
...
"syn match csFunCall "\<[A-Z][_$a-zA-Z0-9]*\>" "nextgroup=csFunCallArgs
"syn region csFunCallArgs start="\(" end="\)" contained contains=scalaConstructorSpecializer
syn match    csParen 					"(" "contains=cParen,cCppParen
syn match    csFunCall					"\w\+\s*(" contains=csParen
...
syn keyword csRepeat					break continue do for foreach goto while
...
" constant
syn keyword csConstant					false null true this
" exception
syn keyword csException					try catch finally throw
" using
syn keyword csInclude					using

syn keyword csKeyword					as is in new yield return
" TODO:
syn keyword csUnspecifiedStatement		base checked event fixed lock
syn keyword csUnspecifiedStatement      new operator out params ref sizeof
syn keyword csUnspecifiedStatement      stackalloc typeof unchecked unsafe
"syn keyword csUnspecifiedStatement      return yield
" TODO:
...
syn region	csDebug			start="^.*Debug" end=";" contains=csFunCall
...
hi def link csFunCall			Function
